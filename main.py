...
# 1. Authenticated CDP Facilitator for Base Mainnet
server = x402ResourceServer(HTTPFacilitatorClient(create_facilitator_config()))
server.register(NETWORK, ExactEvmServerScheme())

# 2. Define Payment Gated Routes using Official RouteConfig
routes = {
    "GET /v1/intel/pulse/*": RouteConfig(
        accepts=[
            PaymentOption(scheme="exact", pay_to=RECEIVER, price="$0.02", network=NETWORK)
        ],
        mime_type="application/json",
        description="Tier 1: 5m Momentum and Volume Pulse",
    ),
    "GET /v1/intel/orderbook/*": RouteConfig(
        accepts=[
            PaymentOption(scheme="exact", pay_to=RECEIVER, price="$0.05", network=NETWORK)
        ],
        mime_type="application/json",
        description="Tier 2: Orderbook Depth and Slippage Analysis",
    ),
    "GET /v1/intel/whale-flow/*": RouteConfig(
        accepts=[
            PaymentOption(scheme="exact", pay_to=RECEIVER, price="$0.10", network=NETWORK)
        ],
        mime_type="application/json",
